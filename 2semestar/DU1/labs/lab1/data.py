...
def eval_perf_multi(Y, Y_):
    ...
    Args:
        Y (numpy.ndarray): Ground truth class indices.
        Y_ (numpy.ndarray): Predicted class indices.

    Returns:
        tuple: (accuracy, per-class precision/recall, confusion matrix)
    """
...
def sample_gmm_2d(K, C, N):
    ...
    for i in range(K):
        ...
        Y.append(np.random.randint(C))
    
    X = np.vstack([x.get_sample(N) for x in X])
    Y_ = np.hstack([[y]*N for y in Y])
