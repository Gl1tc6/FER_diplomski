...
class SoftmaxCrossEntropyWithLogits():
  ...
  #   # DONE
  #   n, _ = x.shape
  #   self.probs = np.exp(x) / np.sum(np.exp(x), axis=1, keepdims=True) # softmax
  #   return (-np.sum(y * np.log(self.probs+ 1e-10), axis=1)) / n # loss
  def forward(self, x, y):
    n, _ = x.shape
    self.probs = np.exp(x - np.max(x, axis=1, keepdims=True))  # Numerical stability
    self.probs /= np.sum(self.probs, axis=1, keepdims=True)  # Softmax
    loss = -np.sum(y * np.log(self.probs + 1e-10)) / n  # Sum over ALL elements and divide by batch size
    return loss
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    # DONE
    return 0.5 * self.weight_decay * np.sum(pow(self.weights,2))
...
class RegularizedLoss():
  ...
  #   return loss_val
  def forward(self, x, y):
    data_loss = self.data_loss.forward(x, y)
    reg_loss = sum([reg.forward() for reg in self.regularizer_losses])
    return data_loss + reg_loss  # scalar 
