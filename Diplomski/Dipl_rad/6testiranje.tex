\chapter{Testiranje detekcije i automatske reakcije}
...
\texttt{notice.log}, preko skripte \texttt{auto\_reaction.py}, do izmjene članstva skupa \texttt{denylist} u tablici \texttt{nftables}. Svaki se test
...
s pokazateljem kompromitacije te \texttt{hping3} za lažiranje izvorišne adrese pri ispitivanju ograničenja.
...
tijekom \emph{3 sata 38 minuta 41 sekunde}. Nijedan obrazac nije premašio pragove definirane u \texttt{local.zeek} (50 različitih portova u pet minuta odnosno 
...
Grafani. Na nadzornoj ploči \ref{fig:dashboard-start} izravno se očitava da podatci pristižu - prikazani su broj veza u minuti, raspodjela protokola
...
Provedena su tri testa detekcije: skeniranje portova, bruteforce SSH prijave i podudaranje s pokazateljem kompromitacije. Svi su izvedeni s klijenta
...
\ref{fig:timeline}, s pripadnim izvorišnim adresama.
...
\texttt{10.0.1.94} \ref{lst:notice-portscan}. Detektor djeluje na događaju \texttt{new\_connection}, pa reagira na pokušaj veze i ne ovisi o
...
Obavijesti su vidljive i na nadzornoj ploči \ref{fig:ssh-grafana}, gdje je za izvor \texttt{10.0.1.94} zabilježeno više detekcija
...
\subsection{Podudaranje s pokazateljem kompromitacije}
...
\texttt{Intel::Notice} \ref{lst:notice-ioc}.

\begin{lstlisting}[caption={Upozorenje o podudaranju s pokazateljem kompromitacije (\texttt{notice.log})}, label={lst:notice-ioc}]
...
između količine obavijesti i odzivnosti te je on odvojen od \texttt{TIMEOUT} kojeg se postavlja za duljinu trajanja blokade unutar nft tablice \texttt{denylist}.
...
prazan; ubrzo nakon detekcije izvorišna se adresa nalazi u skupu, uz preostalo vrijeme do isteka koje očitava jezgra \ref{lst:nft-block}. Skripta
\texttt{auto\_reaction.py} pritom je ispisala odgovarajuću potvrdu \ref{lst:reaction-portscan}.
...
Za odlaznu vezu prema pokazatelju kompromitacije skripta blokira polje \texttt{src} iz obavijesti, a to je \emph{unutarnji} klijent koji je ostvario
...
Blokada je potvrđena i pregledom skupa nakon podudaranja s pokazateljem kompromitacije. U skupu \texttt{denylist} nalazi se unutarnji klijent
\texttt{10.0.1.94}, a ne vanjska adresa indikatora \ref{lst:nft-ioc}.

\begin{lstlisting}[language=bash, caption={Skup \texttt{denylist} nakon podudaranja s pokazateljem kompromitacije},label={lst:nft-ioc}]
    table ip diplomski {
            set denylist {
                    type ipv4_addr
                    timeout 30s
...
\texttt{denylist} ostao je prazan, a skripta nije poduzela reakciju \ref{lst:whitelist}. Time je potvrđeno da popis dopuštenih adresa
...
skup \texttt{denylist} \ref{lst:spoof}.
