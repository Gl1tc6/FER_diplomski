\chapter{Implementacija sustava u \mbox{Docker} okruženju}

U ovom se poglavlju opisuje konkretna izvedba sustava čija je arhitektura iznesena u prethodnom poglavlju. Redom se prikazuju postavljanje Docker okruženja i način na koji se servisi 
pokreću jednom skriptom, konfiguracija mreže s pristupnom točkom, prevođenjem adresa i pravilima vatrozida te postavljanje Zeek senzora s vlastitim detektorima. Nakon toga opisuju se dva 
podsustava za pohranu, Telegraf s InfluxDB-om i Filebeat s Elasticsearchom, te njihova zajednička vizualizacija u Grafani. Poglavlje završava skriptom za automatsku reakciju i pregledom 
najznačajnijih problema na koje se nailazilo tijekom implementacije.

\section{Postavljanje Docker okruženja}

Svi servisi sustava, osim skripte za automatsku reakciju, izvode se u Docker kontejnerima \cite{docker_docs}. Kreirane su dvije \texttt{docker-compose} datoteke, jedna za svaki
...
port (\texttt{3000} za Grafanu), istovremeno pokretanje nije moguće, odnosno u bilo kojem danom trenutku se može izvoditi samo jedan podsustav, što je ujedno i preduvjet za poštenu usporedbu
...
\texttt{.env} datoteku okruženja, čime se osjetljivi i okruženju specifični parametri drže odvojeno od konfiguracije, što je standardna praksa. Struktura datoteke \texttt{.env} prikazana je u ispisu \ref{lst:env}.
\newpage
...
Pri prvom pokretanju InfluxDB se inicijalizira automatski putem varijabli \texttt{DOCKER\_INFLUXDB\_INIT\_*} u \texttt{setup} načinu rada, prikazanog u ispisu \ref{lst:influx-init}, što
omogućuje da se bez ručne intervencije stvaraju korisnik, organizacija, spremnik i administratorski token. Postupak se izvodi samo pri prvom pokretanju, dok se pri kasnijima 
postojeći podatci zadržavaju. Time se izbjegava ručno postavljanje baze prilikom prvog pokretanja,  što je veoma bitno za ponovljivost i održivost okruženja.
...
Isti se token prosljeđuje Grafani kroz varijablu okoline kako bi se izvor podataka konfigurirao automatski pri pokretanju \engl{provisioning}, bez ručnog unosa pristupnih podataka
...
provodi korak po korak. Ako pokretanje kontejnera ne uspije, prethodno postavljena mrežna konfiguracija automatski se poništava, što je vidljivo u ispisu \ref{lst:dipl-start}, čime se izbjegava
...
U ispisu su \lstinline|$NET_SCRIPT| putanja do mrežne skripte, a \lstinline|${NC}|, \lstinline|${RED}| i slično oznake boja terminala radi preglednijeg ispisa.
\section{Konfiguracija mreže, NAT-a i pristupne točke}

Potrebna mrežna konfiguracija postavlja se skriptom \texttt{net\_setReset\_rules.sh}, koja prima argument \texttt{set} ili \texttt{reset}. Računalo raspolaže dvama sučeljima - žičanim
...
Za rad u načinu pristupne točke prilagodnik mora zadovoljiti dva uvjeta. Njegov \textit{chipset} mora podržavati način rada pristupne točke \engl{master/AP mode}, a pripadni upravljački
program mora biti dostupan kroz \texttt{nl80211} sučelje jezgre kojim \texttt{hostapd} upravlja prilagodnikom. Poželjno je i da je upravljački program dio jezgre čime se izbjegava
...
pristupne točke \cite{wikidevi_wn722n}. Kasnije inačice (v2 i v3) koriste \textit{chipset} Realtek RTL8188EUS, za koji podrška za način pristupne točke nije uključena u standardnu jezgru, 
već zahtijeva ručno postavljanje neslužbenih upravljačkih programa. Stoga je upravo inačica v1 prikladna za ovu primjenu bez dodatnih zahvata.
...
u ispisu \ref{lst:nm-forward}. Izuzimanje je nužno jer bi \texttt{NetworkManager} inače pokušao samostalno upravljati sučeljem i ulazio u sukob s \texttt{hostapd}-om koji isto sučelje
...
Konfiguracija vatrozida i prevođenja adresa smješteni su u zasebnu \texttt{nftables} tablicu \texttt{diplomski}, što omogućuje uklanjanje postavki jednom naredbom
bez utjecaja na ostatak sustava. Tablica sadrži pravilo maskiranja \engl{NAT} odlaznog prometa podmreže \texttt{10.0.1.0/24} na sučelju \texttt{enp2s0}.

Uz pravilo maskiranja, ista tablica unaprijed priprema infrastrukturu za automatsku reakciju pomoću skupa \texttt{denylist} s vremenskim ograničenjem trajanja elemenata te dva lanca
koja odbacuju promet izvorišnih adresa iz tog skupa na \texttt{forward} i \texttt{input} kukama \engl{hooks} \ref{lst:denylist}. Lanci su postavljeni na prioritet -10. U nftables niži 
broj prioriteta znači ranije izvođenje, pa se ova pravila primjenjuju prije zadanih ufw lanaca, čime odbacivanje stupa na snagu prije nego što ostala pravila propuste vezu. Struktura blokade
...
portovima \texttt{67} i \texttt{68} te usmjeravanje s \texttt{wlan1} na \texttt{enp2s0}. Posljedica te je politike da će se dolazni neželjeni internetski promet odbaciti neovisno o ostalim slojevima.
...
Bežična pristupna točka realizirana je alatom \texttt{hostapd} \cite{hostapd_docs} na sučelju \texttt{wlan1}, uz zaštitu WPA2-PSK sa CCMP protokolom, u ispisu \ref{lst:hostapd}. Ključna
...
međusobni promet ne bi prolazio kroz točku nadzora; isključivanjem izolacije Zeek dobiva vidljivost i nad istok-zapad prometom (promet između uređaja) unutar lokalne mreže.
...
Mrežne parametre postavlja \texttt{dnsmasq} \cite{dnsmasq_docs} prema ispisu \ref{lst:dnsmasq} tako da klijentima na sučelju \texttt{wlan1} dodjeljuje adrese iz raspona
...
Zeek se pokreće kao servis u obje \texttt{docker-compose} datoteke, prema ispisu \ref{lst:zeekcmd}. Za razliku od ostalih servisa koji koriste izoliranu
\texttt{bridge} mrežu, Zeek koristi mrežu domaćina (\texttt{network\_mode: host}), pa mu je fizičko sučelje \texttt{wlan1} izravno dostupno bez zasebnog mapiranja; u izoliranoj mreži kontejner
...
Servis se pokreće naredbom \texttt{zeek -C -i wlan1 local}. Argument \texttt{-C} isključuje provjeru kontrolnih zbrojeva \engl{checksum} paketa što je nužno jer mrežne kartice često
...
\texttt{site/local.zeek}. Volumeni razdvajaju pisanje od čitanja tako što je \texttt{zeek-logs} montiran s pravom pisanja kako bi senzor u njega zapisivao, dok su \texttt{local.zeek}
...
strojno čitanje agenata. 
Učitavanje se u datoteci \texttt{local.zeek} provodi direktivom \texttt{@load}, kojom se uključuju Zeekovi ugrađeni okviri i skeneri pojedinih protokola. Ispis \ref{lst:zeekload} prikazuje učitavanje baznih skenera za promatrane protokole te okvira za obavijesti, čime se određuje koje protokole Zeek raščlanjuje i o kojima vodi zapise.

\begin{lstlisting}[caption={Učitavanje ugrađenih dodataka u \texttt{local.zeek}}, label={lst:zeekload}]
...
Bez učitanih dodataka Zeek promatra manji skup protokola, pa je učitavanjem moguće proširiti opseg praćenja prema potrebama nadzirane mreže.
Uz Zeekov ugrađeni SSH \engl{bruteforce} detektor, u skripti su definirani i vlastite detektori. Ispis \ref{lst:zeek-detect} prikazuje detekciju skeniranja
portova. 
...
Indikatori kompromitacije \engl{Indicators of Compromise} učitavaju se iz datoteke \texttt{intel.dat} putem Zeekovog Intel okvira \cite{zeek_docs}. Adrese se navode u datoteci, a
ne u skripti, čime se popis prijetnji može mijenjati bez izmjene detekcijske logike. 

Svi se zapisi razvrstavaju prema vrsti aktivnosti (\texttt{conn.log}, \texttt{dns.log},
...
redak kao JSON (format \texttt{json\_v2}) i upisuje ga u InfluxDB putem izlaza \texttt{influxdb\_v2}, kojem se pristupni token, organizacija i spremnik predaju putem varijabli okoline.
...
filtriranje i grupiranje u Grafani. Vrijednosti s velikim brojem različitih pojavnosti, poput portova, količine prenesenih okteta i identifikatora veze \texttt{uid}, pohranjuju se kao
...
tumači kao razdjelnik putanje, a u zapisu je ona dio samog naziva ključa, točka se u putanji mora izuzeti, u ispisu \ref{lst:telegraf-schema}.
\newpage
\begin{lstlisting}[caption={Izuzimanje točke u nazivu ključa (\texttt{telegraf.conf})}, label={lst:telegraf-schema}]
...
\texttt{@timestamp}. Zapisi se potom šalju u Elasticsearch i pohranjuju u indeks nazvan prema vrsti i datumu, primjerice \texttt{zeek-conn-2026.05.29}, vidljivo u ispisu \ref{lst:filebeat-input}.
...
mapiranje polja stvara dinamički. Posljedice takvog mapiranja na zauzeće prostora razmatrat će se u poglavlju evaluacije.
...
ponovljivost. Za podsustav s InfluxDB-om opisuje se izvor tipa \texttt{influxdb} s Flux jezikom upita, kojem se organizacija, spremnik i token predaju varijablama okoline, u ispisu
...
\texttt{proto}) te ljestvice najčešćih izvorišnih i odredišnih adresa. Primjer Flux upita, za prikaz broja veza po minuti, dan je u ispisu \ref{lst:flux-panel} gdje postavljamo 
period vremena, filter po polju (grupiranje), izbacivanje nepotrebnih stupaca, učestalost uzorkovanja i naziv povratnog upita; oznake se prije
...
Sigurnosni pregled čine tablica upozorenja iz \texttt{zeek\_notice} s tipom i izvorišnom adresom, vremenski prikaz učestalosti upozorenja te tablica
sumnjivih veza. Potonja izdvaja veze čije stanje (\texttt{conn\_state}) odstupa od urednog dovršetka (\texttt{SF} - normalno uspostavljanje i raskid veze) i grupira ih po stanju i 
izvorišnoj adresi, čime se ističu obrasci poput velikog broja neuspjelih ili odbijenih veza s jednog izvora.
...
Za razliku od ostalih komponenata, skripta za automatsku reakciju \texttt{auto\_reaction.py} izvodi se izravno na računalu domaćinu, a ne u kontejneru, jer mijenja njegova vatrozidna pravila
...
\texttt{id.orig\_h} prema ispisu \ref{lst:reaction-core}. Razlog je što vlastiti detektori postavljaju \texttt{\$src} bez pridružene veze, dok upozorenja vezana uz konkretnu vezu
...
Odgovor se pokreće isključivo za unaprijed određen skup upozorenja (\texttt{TARGET\_NOTICES}) kao pokušaji skeniranja portova, pogađanje SSH lozinke te podudaranja s indikatorima
kompromitacije. Na taj način operator može smanjiti (ili povećati) broj upozorenja ovisno o situaciji i potrebama. Adresa usmjernika i adresa domaćina 
u redovnom radu nalaze se u listi dopuštenih adresa (\texttt{WHITELIST = {"127.0.0.1", "10.0.1.1"}}) kako se ne bi mogla blokirati vlastita infrastruktura.
...
pri postavljanju mreže, u ispisu \ref{lst:denylist}. Skripta mijenja isključivo elemente skupa, ne lance ni pravila. Zbog toga bilo kakav nenadani pad skripte ne ostavlja vatrozidna pravila bez nadzora, 
već se jezgra brine o tome da se pobrišu nakon zadanog vremena.
...
signalom \texttt{SIGKILL} ili  ponestankom memorije, elementi svejedno isteknu sami, a cjelokupno se stanje uklanja brisanjem tablice. Skripta uz to vodi
...
tek ako je naredba \texttt{nft} uspješno izvršena, čime se izbjegava pogrešna evidencija nepostojeće blokade.
\newpage
...
Verzija slike namjerno je fiksirana radi ponovljivosti; u produkcijskoj bi je primjeni trebalo redovito ažurirati na novija izdanja sa sigurnosnim zakrpama.
...
utvrđeno je da \texttt{ufw} presreće DHCP pakete (port \texttt{67/UDP}) i odbacuje ih prije nego dospiju do \texttt{dnsmasq}-a. Problem je otklonjen iznimkom dopuštanja DHCP prometa i
...
Pokretanje \texttt{dnsmasq}-a isprva nije uspijevalo zbog zauzetosti porta \texttt{53} (\texttt{Address already in use}), koji je na povratnim adresama držao sistemski razlučivač
...
Na sloju ulaza zapisi pri prvom pokretanju Telegrafa nisu dospijevali u InfluxDB. Uzrok je bilo pogrešno raščlanjivanje Zeekovih ključeva. Format \texttt{json\_v2} točku tumači
...
U pogledu detekcije isprva je za skeniranje portova učitana ugrađena politika \texttt{policy/misc/scan}. U korištenoj inačici Zeeka je ta politika zastarjela i prestala je
