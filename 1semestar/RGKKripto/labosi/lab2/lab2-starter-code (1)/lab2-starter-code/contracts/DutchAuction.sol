...
contract DutchAuction is Auction {
    ...
    function bid() public payable {
        ...
        require(time() < auctionEnd,"Auction ended");
        require(getAuctionOutcome() == Outcome.NOT_FINISHED, "Auction is finished");

        uint currPrice = initialPrice - time() * priceDecrement;
        require(msg.value >= currPrice, "Amount bidded is too low!");
        if (msg.value > currPrice){
            (bool status, ) = msg.sender.call{value: msg.value-currPrice}("");
