...
contract Auction {
    ...
    function finalize() public {
        ...
        require(getAuctionOutcome() == Outcome.SUCCESSFUL, "Auction wasn't successful");
        if (judgeAddress != address(0)){
            require(msg.sender == judgeAddress || msg.sender == getHighestBidder(), "Only judge or auction winner can call this!");
        }
        (bool success, ) = payable(sellerAddress).call{value: address(this).balance}("");
    ...
    function refund() public {
        // TODO Your code here
        //revert("Not yet implemented");
        require(getAuctionOutcome() == Outcome.NOT_SUCCESSFUL, "Cannot refund successful auction");
        require(getHighestBidder() != address(0), "No highest bidder to refund");

        if (judgeAddress != address(0)){
            require(msg.sender == judgeAddress || msg.sender == sellerAddress, "Only seller and judge can call this");
        }
        
        (bool success, ) = payable(getHighestBidder()).call{value: address(this).balance}("");
